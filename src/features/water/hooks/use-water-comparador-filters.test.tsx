...
import { useWaterComparadorFilters } from './use-water-comparador-filters'
...
describe('useWaterComparadorFilters', () => {
  beforeEach(() => {
  ...
  it('auto-selects first sede, pipe and today dates on mount', async () => {
    renderHook(() => useWaterComparadorFilters(), { wrapper: createWrapper() })
  ...
  it('derives the default measurement point once points load', async () => {
    ...
    const { result } = renderHook(() => useWaterComparadorFilters(), { wrapper: createWrapper() })
  ...
  it('is ready with explicit URL params and defaults agrupacion to hour', async () => {
    ...
    expect(result.current.agrupacion).toBe('hour')
